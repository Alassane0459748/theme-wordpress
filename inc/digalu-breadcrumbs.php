...
if ( ! function_exists( 'digalu_breadcrumbs' ) ) {
    function digalu_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            'breadcrumbs_id'      => '',
            'breadcrumbs_classes' => esc_html( '' ),
            'home_title'          => esc_html__( 'Home', 'digalu' )
        );
        $args    = apply_filters( 'digalu_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html = '';
        $html .= '<ul class="breadcrumb" '.implode( ' ',  $args_el ).'>';
        // Add Homepage link (always present)
        $html .= '<li><a href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '"><i class="fas fa-home"></i>' . esc_attr( $args['home_title'] ) . '</a></li>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<li class="active bread-search">'.esc_html__('Search results for: ','digalu') . get_search_query() . '</li>';
        } elseif ( is_404() ) {
            $html .= '<li>' . esc_html__( 'Error 404', 'digalu' ) . '</li>';
        } elseif ( is_home() ) {
        ...
        if( class_exists('woocommerce') ) {
            ...
            echo apply_filters( 'digalu_breadcrumbs_filter', $html );
        }
